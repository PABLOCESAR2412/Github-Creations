...
export const DynamicStatsConfigurator: React.FC = () => {
  const { stats, username, addComponent } = useEditorStore();
  ...
  const renderContent = () => {
    if (!stats) {
      return (
        <div className="col-span-12 flex flex-col items-center justify-center py-12 text-slate-500 border border-dashed border-slate-700 rounded-xl bg-slate-900/50">
          <Settings2 className="w-8 h-8 mb-3 opacity-50" />
          <p className="text-sm">Search for a GitHub user first to configure dynamic stats.</p>
        </div>
      );
    }
    ...
    return (
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Controls */}
        <div className="col-span-1 space-y-6 bg-black p-5 border border-zinc-800 brutal-shadow font-mono">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2 mb-4">
              <button 
                ...
                className={`p-2 border text-xs font-bold uppercase transition-all ${layout === 'neon-cluster' ? 'bg-[#00ffff] border-[#00ffff] text-black' : 'bg-black border-zinc-700 text-zinc-400 hover:border-zinc-500'}`}
              >
                NEON CLUSTER
              </button>
              <button 
                onClick={() => setLayout('data-matrix')}
                className={`p-2 border text-xs font-bold uppercase transition-all ${layout === 'data-matrix' ? 'bg-[#00ffff] border-[#00ffff] text-black' : 'bg-black border-zinc-700 text-zinc-400 hover:border-zinc-500'}`}
              >
                DATA MATRIX
              </button>
            </div>

            <h3 className="text-sm font-bold text-white mb-3 mt-5 uppercase tracking-widest">[ BADGE_STYLE ]</h3>
            <select 
              value={badgeStyle} 
              onChange={(e) => setBadgeStyle(e.target.value)}
              className="w-full bg-black border border-zinc-700 text-white px-3 py-2 text-sm focus:border-[#00ffff] outline-none uppercase"
            >
          ...
          <div>
            <h3 className="text-sm font-bold text-white mb-3 uppercase tracking-widest">[ CUSTOM_COLORS ]</h3>
            <div className="grid grid-cols-3 gap-3">
              <div className="flex flex-col items-center gap-1">
                <input 
                  ...
                  className="w-10 h-10 cursor-pointer bg-black border border-zinc-700 p-1"
                />
                <span className="text-[10px] text-zinc-400 uppercase">LABEL BG</span>
              </div>
              <div className="flex flex-col items-center gap-1">
                <input 
                  type="color" 
                  ...
                  className="w-10 h-10 cursor-pointer bg-black border border-zinc-700 p-1"
                />
                <span className="text-[10px] text-zinc-400 uppercase">VALUE BG</span>
              </div>
              <div className="flex flex-col items-center gap-1">
                <input 
                  type="color" 
                  ...
                  className="w-10 h-10 cursor-pointer bg-black border border-zinc-700 p-1"
                />
                <span className="text-[10px] text-zinc-400 uppercase">ICON COLOR</span>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="space-y-2">
              {availableStats.map(s => (
                <label key={s.id} className="flex items-center justify-between p-2 bg-zinc-950 border border-zinc-800 cursor-pointer hover:border-zinc-700 transition-colors uppercase">
        ...
        {/* Live Preview Area */}
        <div className="col-span-1 lg:col-span-2 bg-black border border-zinc-800 flex flex-col p-6 overflow-hidden relative brutal-shadow">
          <div className="absolute top-4 left-4 text-[10px] font-mono text-[#00ffff] bg-zinc-950 border border-zinc-800 px-2 py-1 uppercase tracking-widest font-bold">
          ...
          <div className="mt-6 w-full bg-zinc-950 border border-zinc-800 p-4 overflow-auto">
            <div className="text-[10px] text-zinc-500 uppercase tracking-widest font-bold mb-2">&gt; RAW_MARKDOWN_HTML</div>
  ...
  return (
    <div className="bg-black border border-zinc-800 p-6 shadow-2xl space-y-6 brutal-shadow">
      <div className="flex items-center justify-between border-b border-zinc-800 pb-4">
        <div className="flex items-center gap-4">
          <div className="p-2.5 bg-[#00ffff] text-black border border-white">
            <Sparkles className="w-5 h-5" />
          </div>
          <div className="font-mono uppercase">
            <h2 className="text-lg font-black text-white tracking-widest">Dynamic Stats Configurator</h2>
            <p className="text-xs text-zinc-500 mt-1">Build a unique layout from raw live API data.</p>
          </div>
        </div>
      </div>
