...
`;
  ...
  return (
    <div className="bg-black border border-zinc-800 p-6 shadow-2xl relative overflow-hidden brutal-shadow font-mono">
      <div className="flex flex-col md:flex-row gap-8 relative z-10">
        ...
        <div className="flex-1 space-y-6">
          ...
          <div className="bg-zinc-950 p-5 border border-zinc-800 space-y-4">
            <h3 className="text-sm font-bold text-white mb-2 uppercase tracking-widest">&gt; 1. CUSTOMIZE_TEMPLATE</h3>
            
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-xs text-zinc-400 mb-1 block uppercase">BACKGROUND_COLOR</label>
                <div className="flex items-center gap-2 bg-black p-2 border border-zinc-800">
                  ...
                  <span className="text-xs text-zinc-300 font-mono uppercase">{bgColor}</span>
                </div>
              </div>
              <div>
                <label className="text-xs text-zinc-400 mb-1 block uppercase">ACCENT_COLOR</label>
                <div className="flex items-center gap-2 bg-black p-2 border border-zinc-800">
                  ...
                  <span className="text-xs text-zinc-300 font-mono uppercase">{accentColor}</span>
                </div>
              </div>
              <div>
                <label className="text-xs text-zinc-400 mb-1 block uppercase">TEXT_COLOR</label>
                <div className="flex items-center gap-2 bg-black p-2 border border-zinc-800">
                  ...
                  <span className="text-xs text-zinc-300 font-mono uppercase">{textColor}</span>
                </div>
              </div>
              <div>
                <label className="text-xs text-zinc-400 mb-1 block uppercase">BORDER_RADIUS</label>
                <div className="flex items-center gap-2 bg-black p-2 border border-zinc-800 h-[42px] px-3">
                  ...
                  <span className="text-xs text-zinc-300 font-mono uppercase">{borderRadius}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-3">
            <h3 className="text-sm font-bold text-white uppercase tracking-widest">&gt; 2. DEPLOY_INSTRUCTIONS</h3>
            <ol className="list-decimal pl-5 text-sm text-zinc-400 space-y-2 uppercase">
              <li>Create a new folder on your computer.</li>
              <li>Inside it, create a folder called <code className="bg-zinc-900 px-1 text-[#00ffff]">api</code>.</li>
              <li>Inside the <code className="bg-zinc-900 px-1 text-[#00ffff]">api</code> folder, create a file named <code className="bg-zinc-900 px-1 text-[#00ffff]">stats.js</code> and paste the code from the right.</li>
              <li>Install the <a href="https://vercel.com/cli" target="_blank" className="text-[#00ffff] hover:underline">Vercel CLI</a> and run <code className="bg-zinc-900 px-1 text-[#00ffff]">vercel</code> in the root folder.</li>
              <li>Use your new URL in your Markdown: <br/><code className="bg-zinc-900 p-1.5 text-[#00ffff] block mt-2 text-xs">![My Stats](https://tu-proyecto.vercel.app/api/stats?username={username || 'tu-usuario'})</code></li>
            </ol>
          </div>
        </div>

        {/* Right Side: Code Output */}
        <div className="flex-1 flex flex-col bg-black border border-zinc-800 overflow-hidden brutal-shadow">
          <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-800 bg-zinc-950">
            <div className="flex items-center gap-2 uppercase tracking-widest">
              <Terminal className="w-4 h-4 text-zinc-400" />
              <span className="text-[10px] font-bold text-zinc-400">&gt; API/STATS.JS</span>
            </div>
            <button
              onClick={handleCopy}
              className="flex items-center gap-1.5 bg-[#00ffff] hover:bg-white text-black px-4 py-2 transition-colors text-xs font-bold uppercase"
            >
              {copiedCode ? <Check className="w-3.5 h-3.5 text-black" /> : <Copy className="w-3.5 h-3.5" />}
              {copiedCode ? '[ COPIED ]' : '[ COPY_CODE ]'}
            </button>
          </div>
